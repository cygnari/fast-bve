...
int main(int argc, char **argv) {
  ...
  // list of points and other information in a flattened array
  std::vector<std::vector<std::vector<int>>> dynamics_triangles;
  // at level i, each entry is a vector which contains the 3 vertices and the refinement level of the triangle
  std::vector<std::vector<bool>> dynamics_triangles_is_leaf;
  // at level i, if triangle j is a leaf triangle
  std::vector<std::vector<bool>> dynamics_triangles_exists;
  // at level i, if triangle j exists

  std::vector<std::vector<double>> fast_sum_icos_verts;
  // vertices for the fast sum icosahedron
  std::vector<std::vector<std::vector<double>>> fast_sum_icos_tri_info;
  // information about fast sum icos triangles
  std::vector<std::vector<std::vector<int>>> fast_sum_icos_tri_verts;
  // triangles for fast sum icosahedron
  std::vector<std::vector<std::vector<int>>> fast_sum_tree_tri_points(run_information.fast_sum_tree_levels);
  // points inside each triangle
  std::vector<std::vector<int>> fast_sum_tree_point_locs(run_information.fast_sum_tree_levels);
   // triangle each point is in
  std::vector<InteractionPair> fast_sum_tree_interactions;
  // c/p - c/p interactions
  ...
  area_initialize(run_information, dynamics_state, dynamics_triangles, dynamics_areas);
  // finds areas for each point
  vorticity_initialize(run_information, dynamics_state, dynamics_areas, omega);
  // initializes vorticity values for each point
  tracer_initialize(run_information, dynamics_state);
  // initializes tracer values for each point
  MPI_Win_create(&fast_sum_tree_point_locs[0], run_information.fast_sum_tree_levels *
  ...
  if (P > 0) { // make sure all processes have the same number of points
    ...
    if (not points_same) {
      if (ID == 0) {
        throw std::runtime_error("Point counts not same across mpi ranks");
      }
    }
  }
  ...
  MPI_Win_create(&c_1[0], run_information.info_per_point *
                     run_information.dynamics_max_points * sizeof(double),
                 sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &win_c1);
  MPI_Win_create(&c_2[0], run_information.info_per_point *
                     run_information.dynamics_max_points * sizeof(double),
                 sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &win_c2);
  MPI_Win_create(&c_3[0], run_information.info_per_point *
                     run_information.dynamics_max_points * sizeof(double),
                 sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &win_c3);
  MPI_Win_create(&c_4[0], run_information.info_per_point *
                     run_information.dynamics_max_points * sizeof(double),
                 sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &win_c4);
  MPI_Win_create(&dynamics_state[0], run_information.info_per_point *
                     run_information.dynamics_max_points * sizeof(double),
                 sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &win_dynstate);
  MPI_Win_create(&stream_func[0], run_information.dynamics_max_points * sizeof(double),
  ...
  if (run_information.write_stream) {
    convolve_stream(run_information, stream_func, dynamics_state,
                    ...
                    icos_tree, 0, omega);
    sync_updates<double>(stream_func, P, ID, &win_stream, MPI_DOUBLE);
  ...
  if (ID == 0) {
    if (run_information.write_output) {
      ...
      int info;
      std::string name1 = run_information.out_path + "/" + output_filename +
                          "/output_init.csv";
    ...
    if (run_information.write_tris) {
      ...
      std::string name3 = run_information.out_path + "/" + output_filename +
                          "_triangles_init.csv";
      std::string name4 = run_information.out_path + "/" + output_filename +
                          "_tri_counts.csv";
  ...
  if (ID == 0) {
    ...
    std::cout << "initialization time: " << std::chrono::duration_cast<std::chrono::microseconds>(end -begin).count()
  ...
  for (int t = 0; t < run_information.time_steps; t++) { // progress the dynamics
    ...
    if (run_information.use_amr) {
      ...
      if (std::abs(test_area - 4 * M_PI) > pow(10, -8)) {
        throw std::runtime_error("Total area wrong");
      }
    ...
    if (P > 1) {
      // make sure all processes have the same number of points, do amr the same way
      points_same = test_is_same(run_information.dynamics_curr_point_count);
      if (not points_same) {
        if (ID == 0) {
          throw std::runtime_error("point counts not same across processes");
        }
      }
    }
    ...
    sync_updates<double>(c_2, P, ID, &win_c2, MPI_DOUBLE);
    inter_state = c_2;                                       // k_2
    ...
    sync_updates<double>(c_3, P, ID, &win_c3, MPI_DOUBLE);
    inter_state = c_3;                                 // k_3
    ...
    sync_updates<double>(c_4, P, ID, &win_c4, MPI_DOUBLE);
    ...
    if (run_information.use_remesh) {
      ...
      sync_updates<double>(dynamics_state, P, ID,
    ...
    if (run_information.write_stream) {
      ...
      sync_updates<double>(stream_func, P, ID, &win_stream,
                           MPI_DOUBLE);
      for (int i = 0; i < run_information.dynamics_curr_point_count; i++) {
    ...
    if (run_information.write_tris and (ID == 0)) {
      write_triangles(run_information, dynamics_triangles, dynamics_triangles_is_leaf,
                      *write_outs3[writer_index], write_out4);
    }
    if (ID == 0) {
  ...
  if (ID == 0) {
    ...
    std::cout << "dynamics time: " << std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count()
              << " microseconds" << std::endl;
  }
