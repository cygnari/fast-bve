...
void clip_assured_sum(const RunConfig &run_information,
  ...
  std::vector<double> prelim_values(run_information.dynamics_curr_point_count,
                                    0);
  ...
  if (std::abs(prelim_mass - target_mass) < pow(10, -10)) {
    for (int i = 0; i < run_information.dynamics_curr_point_count; i++) {
      dynamics_state[run_information.info_per_point * i + 4 + target_species] =
          prelim_values[i];
    }
  } else {
...
void vorticity_fix(const RunConfig &run_information,
  ...
  std::vector<double> prelim_rel_vor(run_information.dynamics_curr_point_count,
                                     0);
  for (int i = 0; i < run_information.dynamics_curr_point_count; i++) {
...
void enforce_conservation(const RunConfig &run_information,
  ...
  for (int i = 0; i < run_information.tracer_count; i++) {
    reconstruct_safely(run_information, dynamics_state, dynamics_areas,
                       qmins[i + 1], qmaxs[i + 1], target_mass[i + 1], i);
  }
}
