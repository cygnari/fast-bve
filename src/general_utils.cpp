...
double dot_prod(const std::vector<double>& x, const std::vector<double>& y) { // dot product of vectors x and y
    double sum = 0;
...
void scalar_mult(std::vector<double>& x, const double scalar) { // multiplies x by scalar in place, modifies x
    for (int i = 0; i < x.size(); i++) x[i] *= scalar;
}

void vec_add(std::vector<double>& x, const std::vector<double>& y) { // adds y to x in place, modifies x
    assert(x.size() >= y.size());
    for (int i = 0; i < x.size(); i++) x[i] += y[i];
}

void vec_minus(std::vector<double>& x, const std::vector<double>& y) { // subtracts y from x in place, modifies x
    assert(x.size() >= y.size());
    for (int i = 0; i < x.size(); i++) x[i] -= y[i];
}
...
void vec_add2d(std::vector<std::vector<double>>& x, const std::vector<std::vector<double>>& y) {
    assert(x.size() == y.size());
    for (int i = 0; i < x.size(); i++) {
...
void vec_minus2d(std::vector<std::vector<double>>& x, const std::vector<std::vector<double>>& y) {
    assert(x.size() == y.size());
    for (int i = 0; i < x.size(); i++) {
        assert(x[i].size() == y[i].size());
        for (int j = 0; j < x[i].size(); j++) x[i][j] -= y[i][j];
...
void matvecmult(const std::vector<std::vector<double>>& Amat, std::vector<double>& xvec) {
...
std::vector<double> slice(const std::vector<double>& x, const int start, const int stride, const int length) {
...
void vector_copy(std::vector<double>& x, const std::vector<double>& y, const int start, const int length) {
    // copies elements from y into x
    for (int i = 0; i < length; i++) x[start + i] = y[i];
}

void vector_copy2(std::vector<double>& x, const std::vector<double> y, const int start, const int length) {
    // copies elements from y into x
    for (int i = 0; i < length; i++) x[start + i] = y[i];
}

std::vector<double> cross_product(const std::vector<double>& x, const std::vector<double>& y) { // cross product of x and y
    assert (x.size() >= 3);
    assert (y.size() >= 3);
...
std::vector<double> cart_to_sphere(const std::vector<double>& p1) { // turns cartesian coordinates to spherical coordinates
    assert (p1.size() == 3);
...
std::vector<double> sphere_to_cart(const double radius, const double colat, const double lon) { // turns spherical coordinates to cartesian coordinates
...
void project_to_sphere(std::vector<double>& p1, const double radius) { // projects a point to the surface of a sphere of radius, modifies p1
...
double great_circ_dist(const std::vector<double>& p1, const std::vector<double>& p2, const double radius) {
...
double great_circ_dist_sph(const double lat1, const double lat2, const double lon1, const double lon2, const double radius) {
    return radius * acos(std::min(1.0, std::max(-1.0, sin(lat1) * sin(lat2) + cos(lat1) * cos(lat2) * cos(lon2 - lon1))));
}
...
std::vector<double> lat_lon(const std::vector<double>& p1) { // finds latitude and longitude of cartesian point
    assert (p1.size() >= 3);
...
std::vector<double> barycoords(const std::vector<double>& p1, const std::vector<double>& p2, const std::vector<double>& p3, const std::vector<double>& p) {
    ...
    std::vector<double> mat {p1[0], p1[1], p1[2], p2[0], p2[1], p2[2], p3[0], p3[1], p3[2]};
...
std::vector<double> normalized_barycoords(const std::vector<double>& p1, const std::vector<double>& p2, const std::vector<double>& p3, const std::vector<double>& p) {
...
bool check_in_tri(const std::vector<double>& p1, const std::vector<double>& p2, const std::vector<double>& p3, const std::vector<double>& p) { // checks if point p is in triangle
    std::vector<double> bary_coord = barycoords(p1, p2, p3, p);
    if ((bary_coord[0] >= 0) and (bary_coord[1] >= 0) and (bary_coord[2] >= 0)) return true;
    else return false;
}

bool check_in_tri_thresh(const std::vector<double>& p1, const std::vector<double>& p2, const std::vector<double>& p3, const std::vector<double>& p, const double threshold) { // checks if point p is in triangle
    std::vector<double> bary_coord = barycoords(p1, p2, p3, p);
...
std::vector<double> tri_sides(const std::vector<double>& p1, const std::vector<double>& p2, const std::vector<double>& p3) {// finds side lengths of triangle
...
std::vector<double> circum_center(const std::vector<double>& p1, const std::vector<double>& p2, const std::vector<double>& p3, const double radius) {
...
double tri_radius(const std::vector<double>& p1, const std::vector<double>& p2, const std::vector<double>& p3, const std::vector<double>& center) { // finds triangle circumradius
...
void replace(std::vector<int>& vals, const int find, const int replacement) { // replace find in vals with replacement
...
int check_point_exist(const std::vector<std::vector<int>>& parent_points, const int point_count, const int iv1, const int iv2) {
...
int check_in_vec(const std::vector<std::vector<double>>& x, const std::vector<double>& y) { // checks if length 3 vector y is in vector of vectors x
    for (int i = 0; i < x.size(); i++) {
...
std::tuple<int, int> find_leaf_tri(const std::vector<double>& target_point, const std::vector<double>& dynamics_state, const std::vector<std::vector<std::vector<int>>>& dynamics_triangles,
        const std::vector<std::vector<bool>>& dynamics_triangles_is_leaf, const int info_per_point, const int max_level) {
    ...
    if (found_leaf_tri) {
        ...
        for (int i = max_level - 1; i > 0; i--) {
            for (int j = 0; j < 20 * pow(4, i); j++) {
                ...
                if (check_in_tri_thresh(v1, v2, v3, target_point, epsilon)) {
                    if ((i == 6) and (j == 0)) {
                        std::cout << "target points: " << target_point[0] << "," << target_point[1] << "," << target_point[2] << std::endl;
                        std::cout << "barycords: " << bary_cords[0] << "," << bary_cords[1] << "," << bary_cords[2] << std::endl;
                        std::cout << "points: " << iv1 << "," << iv2 << "," << iv3 << std::endl;
