...
int linear_solve(const std::vector<double> &interp_mat, std::vector<double> &b_vec, int size, int nrhs, const int solver) {
  ...
  std::vector<double> interp_mat_copy = interp_mat; // copy matrix, some methods destroy the matrix
  int info;
  if (solver == 1) { // dgesv
...
std::vector<double> barycoords(const std::vector<double> &p1,
  ...
  if (info != 0) {
    throw std::runtime_error("Error in barycentric coordinate computation, line 244");
  }
  return coords;
}
...
std::vector<double> barycoords(const std::vector<double> &p1,
  ...
  int info = linear_solve(mat, coords, 3, 1, 1);
  if (info != 0) {
    throw std::runtime_error("Error in barycentric coordinate computation, line 262");
  }
  return coords;
}
