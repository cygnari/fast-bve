...
void fast_sum_icos_init(IcosTree &icos_tree, const double radius, const bool rotate,
  ...
  icos_tree.icosahedron_vertex_coords.push_back(
      project_to_sphere_2(std::vector<double>{0, 1, phi}, radius)); // 12 starting points
  icos_tree.icosahedron_vertex_coords.push_back(project_to_sphere_2(
      std::vector<double>{0, -1, phi}, radius));
  icos_tree.icosahedron_vertex_coords.push_back(project_to_sphere_2(
      std::vector<double>{0, 1, -phi}, radius));
  icos_tree.icosahedron_vertex_coords.push_back(project_to_sphere_2(
      std::vector<double>{0, -1, -phi}, radius));
  icos_tree.icosahedron_vertex_coords.push_back(project_to_sphere_2(
      std::vector<double>{1, phi, 0}, radius));
  icos_tree.icosahedron_vertex_coords.push_back(project_to_sphere_2(
      std::vector<double>{1, -phi, 0}, radius));
  icos_tree.icosahedron_vertex_coords.push_back(project_to_sphere_2(
      std::vector<double>{-1, phi, 0}, radius));
  icos_tree.icosahedron_vertex_coords.push_back(project_to_sphere_2(
      std::vector<double>{-1, -phi, 0}, radius));
  icos_tree.icosahedron_vertex_coords.push_back(project_to_sphere_2(
      std::vector<double>{phi, 0, 1}, radius));
  icos_tree.icosahedron_vertex_coords.push_back(project_to_sphere_2(
      std::vector<double>{phi, 0, -1}, radius));
  icos_tree.icosahedron_vertex_coords.push_back(project_to_sphere_2(
      std::vector<double>{-phi, 0, 1}, radius));
  ...
  if (rotate) {
    ...
    rot_mat[0][1] = sin(rotate_alph) * sin(rotate_beta) * cos(rotate_gamm) - cos(rotate_alph) * sin(rotate_gamm);
    rot_mat[0][2] = cos(rotate_alph) * sin(rotate_beta) * cos(rotate_gamm) + sin(rotate_alph) * sin(rotate_gamm);
    rot_mat[1][0] = cos(rotate_beta) * sin(rotate_gamm);
    rot_mat[1][1] = sin(rotate_alph) * sin(rotate_beta) * sin(rotate_gamm) + cos(rotate_alph) * cos(rotate_gamm);
    rot_mat[1][2] = cos(rotate_alph) * sin(rotate_beta) * sin(rotate_gamm) - sin(rotate_alph) * cos(rotate_gamm);
...
void lpm_interface(std::vector<double> &active_target_velocities,
  ...
  // once all interactions are performed, synchronize the updates across mpi ranks
